...
case class Present(length: Int, width: Int, height: Int) {
  ...
  def smallestSide(): Tuple2[Int,Int] = {
    val List(a, b) = List(length, height, width).sortWith{_ < _}.take(2)
    (a,b)
  }
...
object Present {
  def Parse(dimensions: String): Present = {
    val Array(l, w, h) = dimensions
      .split('x')
...
object WrappingCalculator {
  def amountOfWrapping(p: Present): Int = {
    p.surfaceArea() + amountOfSlackWrapping(p)
  }

  def amountOfSlackWrapping(p: Present): Int = {
    val (a,b) = p.smallestSide()
    List(a,b).product
  }
}

object RibbonCalculator {
  def AmountOfRibbon(p: Present): Int = {
    RibbonAroundPresent(p) + RibbonForBow(p)
  }

  private def RibbonAroundPresent(p: Present): Int = {
    def perimeter(faceLength: Int, face2Length: Int): Int =
      2 * faceLength + 2 * face2Length

    val (a, b) = p.smallestSide()
    perimeter(a, b)
  }

  def RibbonForBow(p: Present): Int = p.volume
}

object Day2Solution {
  def TotalAmountOfWrapping(): Int = {
    val presents = ReadPresents()

    presents.foldLeft(0)((result, p) =>
      result + WrappingCalculator.amountOfWrapping(p))
  }
  ...
  def TotalAmountOfRibbon(): Int = {
    val presents = ReadPresents()
    presents.foldLeft(0)((result, p) =>
      result + RibbonCalculator.AmountOfRibbon(p))
  }

  private def ReadPresents(): List[Present] = {
    val input = InputReader.ReadInput("/Day2.txt").split("\n")
    input.map(Present.Parse).toList
  }
}
